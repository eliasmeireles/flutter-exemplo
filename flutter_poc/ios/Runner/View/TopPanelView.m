...
- (void)updateFrame
{
    ...
    self.logo.frame = CGRectMake((self.frame.size.width-120)/2, 40, 120, TOP_BTN_LEN);
}
...
- (UIImageView*)logo
{
    if (!_logo)
    {
        ...
        _logo.image = [UIImage imageNamed:@"AppIcon"];
...
- (UIButton*)leaveBtn
{
    if (!_leaveBtn)
    {
        ...
        [_leaveBtn setTitle:NSLocalizedString(@"zoomUiSair", @"") forState:UIControlStateNormal];
...
- (void)onTopButtonClicked:(UIButton *)sender
{
    switch (sender.tag) {
        case kTagButtonEnd:
        {
            ...
            [alertController addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"zoomUiSairReuniao", @"")
            ...
            if ([self.actionPresenter isMeetingHost]) {
                [alertController addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"zoomUiEncerrarReuniao", @"")
                                                                    style:UIAlertActionStyleDefault
                                                                  handler:^(UIAlertAction *action) {
