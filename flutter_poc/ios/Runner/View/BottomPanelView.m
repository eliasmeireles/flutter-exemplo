...
- (void)updateFrame
{
    ...
    self.shareButton.frame = CGRectMake(panelWidth*2/4, 0, panelWidth/4, bottomButtonHeight);
    ...
    self.chatButton.frame = CGRectMake(panelWidth*3/4, 0, panelWidth/4, bottomButtonHeight);
    ...
    /*self.moreButton.frame = CGRectMake(panelWidth*4/4, 0, panelWidth/4, bottomButtonHeight);
...
- (UIButton*)audioButton
{
    if (!_audioButton)
    {
        ...
        [_audioButton setTitle:NSLocalizedString(@"zoomUiMute", @"") forState:UIControlStateNormal];
...
- (UIButton*)videoButton
{
    if (!_videoButton)
    {
        ...
        [_videoButton setTitle:NSLocalizedString(@"zoomUiVideoStop", @"") forState:UIControlStateNormal];
...
- (UIButton*)shareButton
{
    if (!_shareButton)
    {
        ...
        [_shareButton setTitle:NSLocalizedString(@"zoomUiShare", @"") forState:UIControlStateNormal];
...
- (UIButton*)chatButton
{
    if (!_chatButton)
    {
        ...
        [_chatButton setTitle:NSLocalizedString(@"zoomUiParticipantes", @"") forState:UIControlStateNormal];
        ...
        case kTagButtonAudio:
        {
            if (ms)
            {
                if (!ms.canUnmuteMyAudio) {
                    ...
                    UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:NSLocalizedString(@"zoomAudioDesabilitadoMsg", nil) preferredStyle:UIAlertControllerStyleAlert];
...
//                    ZoomUtil *zu = [[ZoomUtil alloc] init];
//                    if ([zu microfoneWithController:[[appDelegate window] rootViewController]]) {
//                        [self.audioPresenter muteMyAudio];
//                    } else {
//                        UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"tituloMicrofoneNaoDisponivel", nil) message:NSLocalizedString(@"msgMicrofoneNaoDisponivel", nil) preferredStyle:UIAlertControllerStyleAlert];
//                        [alert addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"ok", nil) style:UIAlertActionStyleCancel handler:nil]];
//                        [[[appDelegate window] rootViewController] presentViewController:alert animated:YES completion:nil];
//                    }
                }
        ...
        case kTagButtonVideo:
        {
            if (ms)
            {
                if (!ms.canUnmuteMyVideo) {
                    ...
                    UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:NSLocalizedString(@"zoomVideoDesabilitadoMsg", nil) preferredStyle:UIAlertControllerStyleAlert];
                    [alert addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"ok", nil) style:UIAlertActionStyleCancel handler:nil]];
                    [[[appDelegate window] rootViewController] presentViewController:alert animated:YES completion:nil];
                } else {
                    [self.videoPresenter muteMyVideo];
//                    ZoomUtil *zu = [[ZoomUtil alloc] init];
//                    if ([zu cameraWithController:[[appDelegate window] rootViewController]]) {
//                        [self.videoPresenter muteMyVideo];
//                    } else {
//                        UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"tituloCameraNaoDisponivel", nil) message:NSLocalizedString(@"msgCameraNaoDisponivel", nil) preferredStyle:UIAlertControllerStyleAlert];
//                        [alert addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"ok", nil) style:UIAlertActionStyleCancel handler:nil]];
//                        [[[appDelegate window] rootViewController] presentViewController:alert animated:YES completion:nil];
//                    }
                }
        ...
        case kTagButtonShare:
        {
            if (ms)
            {
                if (ms.isShareLocked) {
                    ...
                    UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:NSLocalizedString(@"zoomShareDesabilitadoMsg", nil) preferredStyle:UIAlertControllerStyleAlert];
                    [alert addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"ok", nil) style:UIAlertActionStyleCancel handler:nil]];
                    [[[appDelegate window] rootViewController] presentViewController:alert animated:YES completion:nil];
                } else {
...
- (void)updateMyAudioStatus
{
    ...
    if (audioType == MobileRTCAudioType_None)
    {
        ...
        [self.audioButton setTitle:NSLocalizedString(@"zoomUiAudio", @"") forState:UIControlStateNormal];
    }
    else
    {
        if([ms isMyAudioMuted])
        {
            ...
            [self.audioButton setTitle:NSLocalizedString(@"zoomUiNoMute", @"") forState:UIControlStateNormal];
        ...
        else
        {
            if (audioType == MobileRTCAudioType_Telephony) {
                ...
                [self.audioButton setTitle:NSLocalizedString(@"zoomUiMute", @"") forState:UIControlStateNormal];
            } else {
...
- (void)updateMyVideoStatus
{
    ...
    if(!mute)
    {
        ...
        [self.videoButton setTitle:NSLocalizedString(@"zoomUiVideo", @"") forState:UIControlStateNormal];
    }
    else
    {
        [self.videoButton setImage:[UIImage imageNamed:@"icon_meeting_video"] forState:UIControlStateNormal];
        [self.videoButton setTitle:NSLocalizedString(@"zoomUiVideoStop", @"") forState:UIControlStateNormal];
    }
    [self.videoButton setTitleEdgeInsets:UIEdgeInsetsMake(self.videoButton.imageView.frame.size.height ,-self.videoButton.imageView.frame.size.width, 0.0,0.0)];
    [self.videoButton setImageEdgeInsets:UIEdgeInsetsMake(-self.videoButton.imageView.frame.size.height/2, 0.0,0.0, -self.videoButton.titleLabel.bounds.size.width)];
}
...
- (void)updateMyShareStatus
{
    ...
    if (ms.isStartingShare)
    {
        ...
        [self.shareButton setTitle:NSLocalizedString(@"zoomUiShareStop", @"") forState:UIControlStateNormal];
    } else {
        [self.shareButton setImage:[UIImage imageNamed:@"icon_meeting_share"] forState:UIControlStateNormal];
        [self.shareButton setTitle:NSLocalizedString(@"zoomUiShare", @"") forState:UIControlStateNormal];
    }
    [self.shareButton setTitleEdgeInsets:UIEdgeInsetsMake(self.shareButton.imageView.frame.size.height ,-self.shareButton.imageView.frame.size.width, 0.0,0.0)];
    [self.shareButton setImageEdgeInsets:UIEdgeInsetsMake(-self.shareButton.imageView.frame.size.height/2, 0.0,0.0, -self.shareButton.titleLabel.bounds.size.width)];
}
